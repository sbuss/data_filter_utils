...
range_filter = lambda line: (
    int(line['response_time']) < 200 or int(line['response_time']) > 1500)
...
def summarize_simon(filename, std_dev_filter=None):
    ...
    filters = [center_filter, range_filter]
    if std_dev_filter:
...
def summarize_all_simon(dirname):
    ...
    std_dev_filter = get_dir_mean_std_filter(
        ...
        exclude_lines=24, filters=[center_filter, range_filter])
